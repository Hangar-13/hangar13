...
export default async function ManagerDashboardPage() {
  ...
  return (
    <DashboardPageShell>
      ...
      <DashboardContentFrame>
        <div className="space-y-1">
          <DashboardSectionLabel>Top training paths</DashboardSectionLabel>
            ...
            <table className="w-full min-w-[36rem] text-sm">
              <thead>
                <tr className="border-b bg-muted/30 text-left text-muted-foreground">
                  ...
                  <th className="p-3 pr-4 font-medium">Name</th>
                  <th className="p-3 pr-4 font-medium whitespace-nowrap"># Students</th>
                  <th className="p-3 font-medium whitespace-nowrap">Last update</th>
                </tr>
              </thead>
              <tbody>
                {topMaterials.map((row) => (
                  <tr key={row.id} className="border-b border-border/60 last:border-0">
                    <td className="p-3 pr-4 align-top text-muted-foreground">{row.rank}</td>
                    <td className="p-3 pr-4 align-top font-medium">
                      <Link
                        href={row.href}
                    ...
                    <td className="p-3 pr-4 align-top tabular-nums">{row.studentCount}</td>
