-- Users list: replace flat org / lead strings with JSON array { name, is_lead } per membership.
...
COMMENT ON FUNCTION public.god_list_users_paginated(text, int, int) IS
  'God dashboard: list users with search and pagination. organizations: [{name, is_lead}]. Caller must be system role god.';
