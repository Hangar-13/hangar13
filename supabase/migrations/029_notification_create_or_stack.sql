-- Create or stack a notification. Runs with SECURITY DEFINER to bypass RLS
-- so that User A can create/update notifications for User B (recipient).
...
BEGIN
  ...
  -- Check for existing notification (we bypass RLS in this function)
  ...
  -- Build message based on type and count
...
-- Allow authenticated users to call this function (server actions use anon key with user JWT)
GRANT EXECUTE ON FUNCTION public.create_or_stack_notification(UUID, TEXT, UUID, TEXT, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_or_stack_notification(UUID, TEXT, UUID, TEXT, UUID) TO anon;
